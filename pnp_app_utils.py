...
def relu(Z):
    ...
    ###  cache -- aApython dictionary containing "A" ; stored for computing the backward pass efficiently
...
def sigmoid_backward(dA, cache):
    ...
    ###  Arguments:
    ###  dA - Post-activation gradient, of any shape
    ###  cache - 'Z' storage for computing backward propagation efficiently
...
def initialize_parameters(n_x, n_h, n_y):
    ...
    ### Initializes all the parameters needed to run forward and backward propagation
...
def compute_cost_with_regularization(AL, Y, parameters, lambd):
    ...
    # Calculate all the squared sums of all W factors
    for l in range(1, L):
        w_name = parameters["W" + str(l)]
        total_sum += np.sum(np.square(w_name))

    # Calculate regularization portion of cost   
    L2_regularization_cost = (1/m) * (lambd/2) * total_sum

    # Add cross entropy cost plus regularization portion for total cost
    cost = cross_entropy_cost + L2_regularization_cost
    
    return cost


def linear_backward_with_reg(dZ, lambd, cache):
...
def linear_activation_backward_with_reg(dA, lambd, cache, activation):
    ...
    if activation == "relu":
        ...
        dA_prev, dW, db = linear_backward_with_reg(dZ, lambd, linear_cache)
...
def L_model_backward_with_reg(AL, Y, caches, lambd):
    ...
    ###  Y -- True "label" vector (containing 0 if no person, 1 if there is a person)
    ###  caches -- List of caches containing:
    ###              every cache of linear_activation_forward() with "relu" (there are (L-1) or them, indexes from 0 to L-2)
    ###              the cache of linear_activation_forward() with "sigmoid" (there is one, index L-1)
    ###  lambd -- Lambda value for regularization
    ...
    grads["dA" + str(L)], grads["dW" + str(L)], grads["db" + str(L)] = linear_activation_backward_with_reg(dAL, lambd, current_cache, activation = "sigmoid")
    
    for l in reversed(range(L-1)):
        # lth layer: (RELU -> LINEAR) gradients.
        current_cache = caches[l]
        dA_prev_temp, dW_temp, db_temp = linear_activation_backward_with_reg(grads["dA" + str(l + 2)], lambd, current_cache, activation = "relu")
...
def predict(X, y, parameters):
    ...
    ###  Arguments:
    ###  X -- Data set of examples to predict on
